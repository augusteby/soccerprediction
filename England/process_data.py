...
def generate_results_hometeam_current_season(team_name, current_date, 
    ...
    selected_results['h_last_n_games_points'] = results_home['last_n_games_home_points']
...
def get_home_results_current_season(team_name, previous_games, n=3): 
    ...
    if nb_home_games>0:
        results['nb_home_victories'] = len(previous_home_games[previous_home_games['FTR']=='H'].index)
        results['nb_home_draws'] = len(previous_home_games[previous_home_games['FTR']=='D'].index)
        
        last_n_home_games = previous_home_games.tail(n)
        last_n_home_victories = len(last_n_home_games[last_n_home_games['FTR']=='H'].index)
        last_n_home_draws = len(last_n_home_games[last_n_home_games['FTR']=='D'].index)
        ...
        results['mean_nb_home_goals_scored'] = float(results['nb_home_goals_scored'])/nb_home_games
        results['nb_home_goals_conceded'] = previous_home_games['FTAG'].sum()
        results['mean_nb_home_goals_conceded'] = float(results['nb_home_goals_conceded'])/nb_home_games
...
def get_away_results_current_season(team_name, previous_games, n=3): 
    ...
    if nb_away_games>0:
        results['nb_away_victories'] = len(previous_away_games[previous_away_games['FTR']=='A'].index)
        results['nb_away_draws'] = len(previous_away_games[previous_away_games['FTR']=='D'].index)
        
        last_n_away_games = previous_away_games.tail(n)
        last_n_away_victories = len(last_n_away_games[last_n_away_games['FTR']=='A'].index)
        last_n_away_draws = len(last_n_away_games[last_n_away_games['FTR']=='D'].index)
        ...
        results['mean_nb_away_goals_scored'] = float(results['nb_away_goals_scored'])/nb_away_games
        results['nb_away_goals_conceded'] = previous_away_games['FTHG'].sum()
        results['mean_nb_away_goals_conceded'] = float(results['nb_away_goals_conceded'])/nb_away_games
...
def generate_results_awayteam_current_season(team_name, current_date, 
    ...
    selected_results['a_nb_victories_draws'] = results_away['nb_away_victories'] + results_away['nb_away_draws']
